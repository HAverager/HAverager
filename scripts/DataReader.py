...
def paverage(_bins, _data, _error):
	...
	for cdf in listofdfs:
		...
		for index, row in row_iterator:  # Over every row of every dataframe in listofdfs
			...
			tserror2 = np.nan_to_num(
				tserror1)  # Clean up any possible "NaN" induced by missing values for error columns (i.e. if error is not present)
...
# This routine parses all .csv data in the current directory for the Fortran routine average. 
	...
	# Read all .csv in current directory and make a list of dataframes
	listofdfs = []
	for i, fileloc in enumerate(files):
		print fileloc
		file = pd.read_csv(fileloc)
		file.insert(0, 'nFile', i)
		listofdfs += [file]
	...
	# Check input parameters. If they are not given, use default
	data = 'data'
	binnames = [s for s in fields if "bin" in s]
	serror = [s for s in fields if "stat" in s]
	oerror = [s for s in fields if ("stat" not in s)
				  and ("bin" not in s)
				  and ("data" not in s)
				  and ("nFile" not in s)]

	# Reordr fields
	newFields = ["nFile"] + binnames + [data] + serror + oerror
	df = df[newFields]

	# Group the mega-dataframe by the binnames and
	# extract the bin names from the grouped dataframes. 
	grouped = list(df.groupby(binnames))

	A = np.array(list(df.groupby(binnames).apply(pd.DataFrame.as_matrix)))

	bins = [item[0] for item in grouped]

	# If the bin names are tupples - i.e. if more than one column specifies the bin.
	if(type(bins[0])==tuple):
		bins = [list(row) for row in bins]   # Convert tuples to lists.

	dataAll = np.zeros((len(files),A[0].shape[1],A.shape[0]))

	for i, line in enumerate(A):
		dataAll[line[:,0].astype(int), :, i] = line

	dataAll =  np.nan_to_num(dataAll)

	data_ = dataAll[:,len(binnames)+1,:].T
	serror_ = dataAll[:,len(binnames)+2,:].T
	oerror_ = dataAll[:,len(binnames)+3:,:].swapaxes(0,1).swapaxes(1,2)
